...
for path in sys.path:
    if "DiffSynth-Studio/scripts" in path:
...
from diffsynth.models.wan_video_dit import WanModel, RMSNorm, sinusoidal_embedding_1d, DiTBlock
...
class ControlNet_DiT(torch.nn.Module):

    def __init__(self, num_layers):
        super(ControlNet_DiT, self).__init__()
...
class ControlNet_PatchEmbedding(torch.nn.Module):
    """
    A module that maps a 3D tensor to a target sequence shape.

    Input: (bs, 16, 13, 60, 104)
    Output: (bs, 20280, 5120)
    """
    def __init__(self, in_channels=16, dim=5120, patch_size=(1, 2, 2)):
        super().__init__()

        # Wan2.2 default params: (*, 5120, [1, 2, 2], [1, 2, 2])
        self.patch_embedding = torch.nn.Conv3d(in_channels, dim, kernel_size=patch_size, stride=patch_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.patch_embedding(x) # (bs, 13, 60, 104, 32) --> (bs, 5120, 13, 30, 52)
        x = rearrange(x, 'b c t h w -> b (t h w) c').contiguous() # (1, 20280, 5120)
...
class ControlNet(torch.nn.Module):
    """
    Contains all trainable parameters for the controlnet
    """
    def __init__(self, num_layers, stride=None, torch_dtype=torch.bfloat16):
        super().__init__()

        self.num_layers = num_layers
        self.stride = stride

        self.controlnet_patch_embedding = ControlNet_PatchEmbedding().to(torch_dtype)

        self.controlnet_dit = ControlNet_DiT(self.num_layers)

        # Same number of input and output channels
        in_channels = 5120
        out_channels = 5120
        self.controlnet_zero_convs_after = torch.nn.ModuleList([
            zero_module(torch.nn.Conv1d(in_channels, out_channels, kernel_size=1, dtype=torch_dtype))
            for _ in range(num_layers)
        ])
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None, controlnet=False, controlnet_num_layers=0, controlnet_stride=None):
        super().__init__(
        ...
        self.units = [
            ...
            WanVideoUnit_ControlVideoEmbedder(), # Nate added this one
        ...
        if controlnet:
            # initialize controlnet in pipeline
            self.num_layers = controlnet_num_layers
            self.controlnet = ControlNet(self.num_layers, stride=controlnet_stride)
            self.in_iteration_models += ("controlnet",)
            self.in_iteration_models_2 += ("controlnet2",) # this one is only used during inference
    ...
    def load_controlnet_weights(self, module, path, torch_dtype):
        state_dict = load_state_dict(path, torch_dtype=torch_dtype)
        module.load_state_dict({k.replace('pipe.controlnet.', '', 1): v for k, v in state_dict.items()}, strict=True)
    ...
    def enable_vram_management(self, num_persistent_param_in_dit=None, vram_limit=None, vram_buffer=0.5):
        ...
        if hasattr(self, 'controlnet') and self.controlnet is not None:
            print("Enabling VRAM management for ControlNet.")
            dtype = next(iter(self.controlnet.parameters())).dtype
            device = "cpu" if vram_limit is not None else self.device
            enable_vram_management(
                self.controlnet,
                module_map = {
        ...
        if hasattr(self, 'controlnet2') and self.controlnet2 is not None:
            print("Enabling VRAM management for ControlNet2.")
            dtype = next(iter(self.controlnet2.parameters())).dtype
            device = "cpu" if vram_limit is not None else self.device
            enable_vram_management(
                self.controlnet2,
                module_map = {
    ...
    def from_pretrained(
        ...
        pipe = WanVideoPipeline(
            device=device, torch_dtype=torch_dtype, controlnet=controlnet, 
        ...
        if controlnet:
            if apply_strided_controlnet:
                # Get the total number of layers from the main DiT model
                total_dit_layers = len(dit.blocks) if not isinstance(dit, list) else len(dit[0].blocks)

                if controlnet_stride is None:
                    assert controlnet_num_layers > 0, "Either controlnet_num_layers or controlnet_stride should be a valid positive integer when using strided controlnet."

                    controlnet_stride = math.ceil(total_dit_layers / controlnet_num_layers)
                else:
                    # Ensure ControlNet stride is in range
                    assert controlnet_stride > 0, "controlnet_stride should be a valid positive integer."
                    assert controlnet_stride < total_dit_layers, "controlnet_stride should be smaller than the total number of DiT layers."

                    # Calculate how many ControlNet layers we need based on stride and reinitialize the ControlNet
                    pipe.num_layers = math.ceil(total_dit_layers / controlnet_stride)
                    
                pipe.controlnet = ControlNet(pipe.num_layers, stride=controlnet_stride)
            else:
                pipe.controlnet.stride = None  # ensure stride is None when apply_strided_controlnet is False, this affects forward pass
                controlnet_stride = 1

            # initialize the weights of the DiT controlnet layers using original weights.
            if isinstance(dit, list):
                # first controlnet gets weights for first dit
                for block_id in range(pipe.controlnet.controlnet_dit.num_layers):
                    pipe.controlnet.controlnet_dit.blocks[block_id] = copy.deepcopy(dit[0].blocks[block_id * controlnet_stride])

                # second controlnet gets weights for second dit
                pipe.controlnet2 = copy.deepcopy(pipe.controlnet)
                # pipe.controlnet.controlnet_dit2 = copy.deepcopy(pipe.controlnet.controlnet_dit)
                for block_id in range(pipe.controlnet2.controlnet_dit.num_layers):
                    pipe.controlnet2.controlnet_dit.blocks[block_id] = copy.deepcopy(dit[1].blocks[block_id * controlnet_stride])
            else:
                for block_id in range(pipe.controlnet.controlnet_dit.num_layers):
                    pipe.controlnet.controlnet_dit.blocks[block_id] = copy.deepcopy(dit.blocks[block_id * controlnet_stride])
    ...
    def __call__(
        ...
        inputs_shared = {
            ...
            "control_signal_video" : control_signal_video
        }
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if timestep.item() < switch_DiT_boundary * self.scheduler.num_train_timesteps and self.dit2 is not None and not models["dit"] is self.dit2:
                ...
                if controlnet:
                    models["controlnet"] = self.controlnet2
                    print("--> Computation is at switch_DiT_boundary, so we loaded dit2 and controlnet2. <--")
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        input_video = pipe.preprocess_video(input_video) # (1, 3, 49, 480, 832), floats in [-1, 0.5547]
        input_latents = pipe.vae.encode(input_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # (1, 16, 13, 60, 104)
        if vace_reference_image is not None:
            vace_reference_image = pipe.preprocess_video([vace_reference_image])
            vace_reference_latents = pipe.vae.encode(vace_reference_image, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
            input_latents = torch.concat([vace_reference_latents, input_latents], dim=2)
        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents} # both shape (1, 16, 13, 60, 104)
        else:
...
class WanVideoUnit_ControlVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("control_signal_video", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, control_signal_video, tiled, tile_size, tile_stride):
        pipe.load_models_to_device(["vae"])
        control_signal_video = rearrange(control_signal_video, 'f h w c -> 1 c f h w') # (1, 3, 49, 480, 832), floats in [-1, 0.9609]
        control_signal_video_latents = pipe.vae.encode(
            control_signal_video, device=pipe.device, tiled=tiled, tile_size=tile_size, 
            tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # (1, 16, 13, 60, 104), floats in [-2.8, 3.1]

        return {"control_signal_video_latents": control_signal_video_latents}
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # first, we figure out whether this model xn is for a controlnet
    ...
    if use_controlnet:
        # encoding strategy using only patch embedding applied to latent control signal
        control_signal_video_latents = kwargs.get("control_signal_video_latents", None) # (1, 16, 13, 60, 104)
        control_signal_encoded = controlnet.controlnet_patch_embedding(control_signal_video_latents)# (1, 16, 13, 60, 104) --> (1, 20280, 5120)
        ...
        controlnet_state = control_signal_encoded

        # forward pass through controlnet to get the hidden states, which we save in a list, and then  use below. 
        controlnet_states = []
        for block_id, block in enumerate(controlnet.controlnet_dit.blocks):
            if use_gradient_checkpointing_offload:
                with torch.autograd.graph.save_on_cpu():
                    controlnet_state = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(block),
                        controlnet_state, context, t_mod, freqs,
                        use_reentrant=False,
                    )
                ...
                # controlnet_state: (1, 20280, 5120)
                # context:          (1, 512, 5120)
            else:
                controlnet_state = block(controlnet_state, context, t_mod, freqs)
            
            controlnet_states.append(controlnet_state)
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if use_controlnet:
                if controlnet.stride is not None: # apply strided ControlNet
                    if (block_id % controlnet.stride == 0) and (block_id // controlnet.stride < len(controlnet_states)):
                        x = x + controlnet_states[block_id // controlnet.stride]
                else:
                    if block_id < controlnet.controlnet_dit.num_layers:
                        # add previously computed controlnet state corresponding to this DiT layer to x, after passing thru zero-conv
                        controlnet_state = controlnet_states[block_id] # (1, 20280, 5120)
                        controlnet_state = rearrange(controlnet_state, 'b a c -> b c a') # (1, 5120, 20280)
                        controlnet_state = controlnet.controlnet_zero_convs_after[block_id](controlnet_state) # (1, 5120, 20280)
                        controlnet_state = rearrange(controlnet_state, 'b c a -> b a c') # (1, 20280, 5120)
                        x = x + controlnet_state
